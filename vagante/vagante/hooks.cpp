...
int init_hooks()
{
	...
	if ((status = MH_CreateHook((LPVOID)(moduleBase + Player$$StepOffset), hPlayer$$Step, (LPVOID*)&oPlayer$$Step)) != MH_OK)
	{
		return status;
	}

	if ((status = MH_CreateHook((LPVOID)(moduleBase + Player$$DeleteOffset), hPlayer$$Delete, (LPVOID*)&oPlayer$$Delete)) != MH_OK)
	{
		return status;
	}
